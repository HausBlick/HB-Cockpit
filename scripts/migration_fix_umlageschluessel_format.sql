-- ── FIX: JAB/WP-Template — korrekte Blockfolge + EUR-Format ──────
-- Behebt 3 Bugs:
-- 1. Umlageschlüssel-Tabelle: fehlende "format":"eur" auf gesamt/anteil
-- 2. Vertauschte Blöcke in der Jahresabrechnung (Headings passten nicht zu Tabellen)
-- 3. Hausgeld-Monatsübersicht zeigte falschen Inhalt

-- ═══ Jahresabrechnung: Komplettes Template mit korrekter Reihenfolge ═══
...
-- ═══ Einzelwirtschaftsplan: Umlageschlüssel-Format fixen ═══
UPDATE pdf_templates SET content = '[
...
updated_at = NOW()
WHERE type = 'einzelwirtschaftsplan';
