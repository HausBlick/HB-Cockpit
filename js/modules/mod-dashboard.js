...
function _dashPersonCard({ name, sub, badge, avatarUrl, phone, mobile, whatsapp, email }) {
    ...
    const line = (href, svg, text, cls) =>
        `<a href="${href}" class="flex items-center gap-2 text-sm mt-1.5 transition-colors ${cls || 'text-gray-600 hover:text-hb-olive'}">${svg}<span class="truncate">${text}</span></a>`;
    const wa = whatsapp ? _waNumber(mobile) : null;
    const hasContact = phone || mobile || email || wa;
    return `
        <div class="snap-start shrink-0 basis-full lg:basis-1/2 p-5">
            <div class="flex gap-4 items-stretch">
                <!-- Links: Bild + Name/Position/Badge -->
                <div class="flex flex-col items-center text-center w-24 shrink-0">
                    ${avatarUrl
                        ? `<img src="${avatarUrl}" class="w-20 h-20 rounded-full object-cover" alt="">`
                        : `<div class="w-20 h-20 rounded-full bg-hb-olive/10 text-hb-olive flex items-center justify-center font-extrabold text-2xl">${initial}</div>`}
                    <div class="font-bold text-hb-offblack text-sm leading-tight mt-2 break-words">${name || '—'}</div>
                    ${sub ? `<div class="text-xs text-gray-500 mt-0.5 break-words">${sub}</div>` : ''}
                    <span class="text-[10px] font-black uppercase bg-hb-olive/10 text-hb-olive px-1.5 py-0.5 rounded mt-1.5 inline-block">${badge}</span>
                </div>
                <!-- Rechts: Kontaktdaten -->
                <div class="flex-1 min-w-0 border-l border-gray-100 pl-4 flex flex-col justify-center">
                    ...
                    ${!hasContact ? '<span class="text-xs text-gray-400">Keine Kontaktdaten hinterlegt.</span>' : ''}
                </div>
            </div>
        </div>`;
}

// F1: Karussell-Scroll (eine Karten-Breite pro Klick; Swipe via nativem Scroll-Snap)
window._anspScroll = (dir) => {
    const el = document.getElementById('ansp-carousel');
    if (!el) return;
...
async function _renderUserDashboard() {
    ...
    if (hausmeister) anspPersons.push({
        ...
        badge: 'Hausmeister', avatarUrl: hausmeister.logo_url,
        phone: hausmeister.phone, mobile: hausmeister.mobile, whatsapp: false, email: hausmeister.email,
    });
    ...
    ca.innerHTML = `
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <div class="card overflow-hidden">
                ...
                ${anspPersons.length ? `
                    <div class="relative">
                        <div id="ansp-carousel" class="flex overflow-x-auto snap-x snap-mandatory scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden divide-x divide-gray-50">
                            ${anspPersons.map(p => _dashPersonCard(p)).join('')}
                        </div>
                        ${anspPersons.length > 1 ? `
                            <button onclick="_anspScroll(-1)" aria-label="Zurück"
                                class="absolute left-1 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/95 shadow-md border border-gray-100 flex items-center justify-center text-hb-olive hover:bg-white transition-colors ${anspPersons.length <= 2 ? 'lg:hidden' : ''}">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7"/></svg>
                            </button>
                            <button onclick="_anspScroll(1)" aria-label="Weiter"
                                class="absolute right-1 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/95 shadow-md border border-gray-100 flex items-center justify-center text-hb-olive hover:bg-white transition-colors ${anspPersons.length <= 2 ? 'lg:hidden' : ''}">
